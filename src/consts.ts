...
export const SITE_DESCRIPTION =
  "ДОКА: автономная система автоматизации ВЭД-логистики. Разбирает инвойсы, подбирает коды ТН ВЭД ЕАЭС тремя ролями (Юрист, Инспектор, Координатор), считает таможенные риски и выгружает проект декларации в Альта-ГТД и 1С:ERP. Стоит на серверах компании, данные наружу не уходят (152-ФЗ). Правообладатель ООО «ГК Фороней».";
...
export const HERO = {
  ...
  pain: "4 часа в Альте на один инвойс. Один неверный код ТН ВЭД даёт КТС и простой.",
  lead: "Спецификация, коды ТН ВЭД ЕАЭС с обоснованием, риски КТС и проект XML за 3–5 минут. Три эксперта работают на ваших серверах (152-ФЗ). Начать можно с разбора 5 файлов за 24 часа, 0 ₽.",
  cta: {
...
export const PAIN_POINTS = [
  ...
  },
  {
    ...
    body: "Ошибка в одной цифре ведёт к доначислениям, штрафам по ст. 16.2 КоАП РФ и спорам с инспектором.",
  },
  {
  ...
  },
  {
    ...
    body: "Данные перекладывают руками между программами. Сквозного пути «PDF → XML → учёт» нет.",
  },
  {
...
export const AUDIENCES = [
  {
    ...
    hook: "ФОТ, сроки поставок, пилот без покупки всей платформы сразу",
...
export const CTA = {
  ...
  href: "/razbor/",
  description: "Отправьте 5 файлов инвойсов. За 24 часа вернём проверку подбора кодов ТН ВЭД, расчёт пошлин и XML-проект декларации.",
};
...
export const CO_AUTHOR = {
  ...
  bio:
    "15 лет в управлении производством, автоматизации и цифровой трансформации. Руководил на предприятиях машиностроения. Занимался и себестоимостью единицы продукции, и стратегией развития. Внедряет автоматизированные решения.",
...
export const SERVICES_STAIRCASE = [
  {
    ...
    deliverables: "Спецификация по 5 инвойсам, коды ТН ВЭД, расчёт пошлин и тестовый XML-проект для Альта-ГТД.",
    result: "На ваших документах видно, насколько точно распознавание и подбор кодов.",
    link: "/razbor/",
    isPrimary: true
  },
  {
  ...
  },
  {
    ...
    price: "от 650 000 ₽",
    duration: "3–6 недель",
    deliverables: "Развёртывание On-Premises (152-ФЗ), интеграция с Альта-ГТД / 1С:ERP, регламенты. В стоимость ПО и внедрения не входят сервер, GPU, СХД: это отдельный CAPEX или аренда заказчика.",
...
export const INFRA_NOTE = {
  ...
  body: "Лицензия и работы по внедрению ДОКА не включают серверы, GPU, СХД и стойку. On-Premises: под vision-модели и пакетную обработку инвойсов нужны вычислительные ресурсы. Если у вас уже есть подходящий хост, используем его. Если нет, фиксируем требования и смету на закупку или аренду. Это CAPEX/OPEX заказчика, в стоимость ПО не входит.",
  bullets: [
    "ПО, внедрение и интеграция входят в прайс ДОКА",
    "Сервер, GPU, диск и сеть идут из бюджета инфраструктуры заказчика",
    "Требования к железу даём после оценки объёма документов (пилот или enterprise)",
  ],
};
...
export const CASE_STUDIES = [
  ...
  },
  {
    ...
    description:
      "Классификация ИТ-номенклатуры: обоснование кода, пошлина/НДС, явный отказ от альтернатив 8471*. Это аргументация для инспектора.",
  ...
  },
  {
    ...
    description:
      "Закрытый контур под 152-ФЗ: локальный разбор, NDA на пилоте, без SaaS-демо «загрузите в интернет». Железо идёт отдельной статьёй CAPEX.",
    tag: "Безопасность",
...
export const PILLARS = [
  {
    title: "Три роли: Юрист, Инспектор, Координатор",
    description:
      "Три независимых контура (Юрист, Инспектор, Координатор) сверяют инвойс с решениями ЕЭК и базой прецедентов ФТС.",
  },
  {
    title: "Распознавание сложных документов и сканов",
    description:
      "Визуальный анализатор читает многостраничные инвойсы, спецификации и MSDS и сохраняет структуру таблиц и артикулов.",
  },
  {
...
export const METHOD_STEPS = [
  {
    ...
    description:
      "Алгоритм читает таблицы, сканы, спецификации и инвойсы без сдвига строк и потери артикулов.",
  },
  {
    num: "02",
    title: "Три роли на каждой позиции",
    description:
      "Юрист проверяет нетарифные ограничения, Инспектор подбирает код ТН ВЭД по базе решений ФТС, Координатор сводит итоговый вердикт.",
  },
  {
    num: "03",
...
export const LONG_LETTER = `ДОКА собирали под декларантов, импортёров и директоров по ВЭД-логистике. На поставках из Китая, Турции и стран Ближнего Востока инвойсы приходят на 30–50 страниц. Таблицы плотные, коды ТН ВЭД либо нет, либо они в зарубежном формате.

Тысячи позиций руками переносят в Альта-ГТД и ищут коды. На одну поставку уходит от нескольких часов до дней. Ошибка в одном знаке кода даёт доначисления, корректировку таможенной стоимости (КТС), простой на СВХ и штрафы по ст. 16.2 КоАП РФ.

Публичные облачные нейросети мы не используем: коммерческие данные наружу не уходят. В ДОКА работают три роли (Юрист, Инспектор, Координатор) по базе решений ФТС РФ и правилам ЕЭК. Система ставится локально, в закрытом контуре компании.

Пришлите 5 файлов инвойсов на бесплатный разбор. За 24 часа отдадим табличную спецификацию с кодами ТН ВЭД, расчётом пошлин и XML-файлом для Альта-ГТД.`;

export const FAQ = [
  {
    question: "Что представляет собой система ДОКА?",
    answer:
      "ДОКА: автономная система автоматизации ВЭД-логистики и таможенного декларирования. Распознаёт многостраничные инвойсы, подбирает коды ТН ВЭД ЕАЭС, оценивает таможенные риски и собирает XML-декларации для Альта-ГТД и 1С:ERP.",
  },
  {
    question: "Как подобрать код ТН ВЭД по инвойсу автоматически?",
    answer:
      "ДОКА извлекает позиции из PDF или скана. Инспектор предлагает код ТН ВЭД ЕАЭС с обоснованием, в том числе почему отброшены альтернативы. Юрист сверяет ограничения, Координатор сводит риски. На выходе спецификация и комментарий для декларанта: как получился код, видно в карточке.",
  },
  {
    question: "Как работают три роли на одном инвойсе?",
    answer:
      "Каждый документ проверяют три независимых контура: Юрист (нормы и ограничения), Инспектор (поиск и обоснование кода ТН ВЭД) и Координатор (оценка рисков КТС). Перекрёстная проверка снижает ошибки, которые обычно даёт один специалист в одиночку.",
  },
  {
  ...
  },
  {
    ...
    answer:
      "На нетиповых или размытых документах визуальный анализатор читает геометрию таблиц. Строки и артикулы остаются на месте.",
  },
  {
    question: "Сколько времени занимает разбор инвойса?",
    answer:
      "Вместо 3–4 часов ручной работы специалиста ДОКА обрабатывает многостраничный инвойс за 3–5 минут и готовит проверенный проект XML-декларации. На тестовых прогонах 32-страничный документ занимает порядка 5–6 минут.",
  },
  {
    question: "С чего начать и что входит в бесплатный разбор 5 файлов?",
    answer:
      "До 5 файлов инвойсов. За 24 часа: демо-выдача распознавания, анализ сложности кодов ТН ВЭД, расчёт пошлин и XML для Альта-ГТД. Контрагентов лучше обезличить; NDA по запросу. Дальше пилот на номенклатуре (от 250 000 ₽), затем Enterprise on-prem (от 650 000 ₽).",
  },
  {
  ...
  },
  {
    ...
    answer:
      "Публичного облачного демо с выносом ваших инвойсов нет. Так сделано из соображений безопасности. Вместо него: бесплатный экспресс-разбор 5 файлов в контролируемом контуре, NDA по запросу, дальше пилот on-prem.",
  },
  {
    question: "Входит ли сервер и GPU в стоимость внедрения ДОКА?",
    answer:
      "Нет. Стоимость ПО, пилота и внедрения это работы и лицензия. Сервер, GPU, СХД, сеть и стойка идут отдельной статьёй CAPEX или аренды заказчика. Если железо уже есть и подходит по нагрузке, используем его. Если нет, после оценки объёма документов выдаём требования и смету на закупку или аренду.",
  },
];
...
export const HOWTO_INVOICE_TO_XML = {
  ...
  description:
    "Четыре шага: извлечение данных из PDF, три роли на позиции, расчёт платежей и рисков КТС, экспорт XML в Альта-ГТД.",
    ...
    author: [
      {
        ...
        description:
          "15 лет в управлении производством, автоматизации и цифровой трансформации. Руководил на предприятиях машиностроения; себестоимость, стратегия развития, внедрение автоматизированных решений.",
      },
      {
        "@type": "Person",
        name: "Наумов Павел Владимирович",
